...
const actions: JourneyAction[] = [
  ...
  },
  {
    ...
    requiresM2M: true,
    requiresUser: "person",
    description: "Cria o perfil de billing Stripe necessário para aceitar ofertas e receber pagamentos. Deve ser executado antes do aceite da oferta (13b).",
    expectedStatus: [200, 201, 400, 409, 500],
    buildBody: state => ({
      email: state.personEmail,
      fullName: String(state.billingFullName || ""),
      taxId: String(state.billingTaxId || ""),
      phone: String(state.billingPhone || ""),
      country: "BR",
    }),
    onSuccess: body => ({
      billingProfileId: findFirst(body, ["id", "billingProfileId", "customerId", "stripeCustomerId"]),
    }),
  },
  {

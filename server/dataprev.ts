...
const actions: JourneyAction[] = [
  ...
  {
    id: "step13_create_billing_profile",
    title: "Criar perfil de billing (Stripe)",
    app: "Personal",
    group: "Marketplace Offers",
    method: "POST",
  ...
  },
  {
    ...
    status: "external",
    requiresM2M: true,
    buildPath: state => `/v1/marketplace/offers/${encodeURIComponent(String(state.selectedOfferId || state.offerId || "MISSING_OFFER_ID"))}/pre-accept`,
    requiresUser: "person",
  ...
  },
  {
    ...
    status: "external",
    requiresM2M: true,
    buildPath: state => `/v1/marketplace/offers/${encodeURIComponent(String(state.selectedOfferId || state.offerId || "MISSING_OFFER_ID"))}/accept`,
    requiresUser: "person",
    ...
    expectedStatus: [200, 201, 204],
    buildBody: state => ({ emailAddress: state.personEmail || "", dataSavingsAccountId: state.dspAccountId || state.dsaId || "" }),
  },
  {
    ...
    status: "external",
    requiresM2M: true,
    buildPath: state => `/v1/marketplace/offers/${encodeURIComponent(String(state.selectedOfferId || state.offerId || "MISSING_OFFER_ID"))}/reject`,
    requiresUser: "person",
...
const steps: JourneyStep[] = [
  ...
  { id: 13, title: "Pessoa aceita/rejeita oferta", app: "Personal", summary: "Criar perfil billing (pré-req) → pré-aceite (opcional) → aceite/rejeição com emailAddress + dataSavingsAccountId.", status: "external", actions: actions.filter(a => a.id === "step13_offer_pre_accept" || a.id === "step13_offer_accept" || a.id === "step13_offer_reject") },
